...
const Discover = ({ posts }: DiscoverProps) => {
  ...
  const filteredPosts =
    activeFilter === FilterId.ALL
      ? posts
      : posts.filter((post) => {
          const activeFilterItem = FILTERS.find(filter => filter.id === activeFilter);
          if (!activeFilterItem) return false;
          return post.attributes.category.some(
            (cat) => cat.toLowerCase().includes(activeFilterItem.slug.toLowerCase())
          );
        });
    ...
    <section className="py-12 bg-[#121212] text-white">
      <div className="container mx-auto px-4">
        <div className="flex flex-col space-y-6 mb-8">
          ...
          <div className="flex flex-wrap gap-2 overflow-x-auto pb-2 no-scrollbar lg:hidden">
            {FILTERS.map((filter) => (
              <button
                key={filter.id}
        ...
        <div className="flex flex-col lg:flex-row gap-8">
          ...
          <div className="lg:w-80 space-y-8">
            <div className="mt-12 mb-6 text-center hidden lg:block">
